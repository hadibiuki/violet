...
export function Input({
  ...
  const affix = leadingAffix || (icon ? React.createElement('i', { ref: iconRef, 'data-lucide': icon, style: { width: 17, height: 17 } }) : null);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--vt-space-2)', fontFamily: 'var(--vt-font-sans)', textAlign: 'start' }}>
      ...
      >
        {affix && <span style={{ display: 'inline-flex', color: 'var(--vt-color-text-muted)' }}>{affix}</span>}
        <input
          id={inputId}
