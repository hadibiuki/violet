...
export function QuantityStepper({ value, onChange, min = 1, max = 9999, step = 1, moq, style, ...rest }: QuantityStepperProps) {
  const lo = moq ?? min;
  const clamp = (n: number) => Math.max(lo, Math.min(max, n));
  const set = (n: number) => onChange && onChange(clamp(n));
